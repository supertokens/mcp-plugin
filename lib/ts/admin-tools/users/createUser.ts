...
const UserRoleSchema = z
  .object({
    ...
    tenantIds: z
      .array(z.string().describe("Tenant ID"))
      .describe("List of tenant IDs associated with the role"),
  })
  .describe("User role with associated tenant IDs");

const TotpDeviceSchema = z
  .object({
    secretKey: z.string().describe("Secret key for TOTP device"),
    skew: z.number().optional().describe("Allowed skew for TOTP in seconds"),
    period: z
      .number()
      .optional()
      .describe("Time period for TOTP code validity in seconds"),
    deviceName: z.string().optional().describe("Name of the TOTP device"),
  })
  .describe("TOTP device configuration");

const BaseLoginMethodSchema = z.object({
  tenantIds: z
    .array(z.string().describe("Tenant ID"))
    .optional()
    .default(["public"])
    .describe(
      "Optional list of tenant IDs to which this login method will be associated"
    ),
  isVerified: z
    .boolean()
    .optional()
    .describe(
      "Optional property which indicates whether the email is verified"
    ),
  isPrimary: z
    .boolean()
    .optional()
    .describe(
      "Optional property that shows whether the login method is the primary login method"
    ),
  timeJoinedInMSSinceEpoch: z
    .number()
    .int()
    .optional()
    .describe(
      "Optional property that specifies the time joined in milliseconds since the epoch"
    ),
});
...
const EmailPasswordLoginMethodSchema = BaseLoginMethodSchema.extend({
  ...
  passwordHash: z.string().optional().describe("Hashed password"),
  plainTextPassword: z
    .string()
    .optional()
  ...
  .refine(
    (value) => value.plainTextPassword || value.passwordHash,
    "Either passwordHash or plainTextPassword must be provided"
  )
...
const ThirdPartyLoginMethodSchema = BaseLoginMethodSchema.extend({
  ...
  thirdPartyId: z.string().describe("Third party provider ID"),
  thirdPartyUserId: z.string().describe("User ID from third party provider"),
...
const PasswordlessLoginMethodSchema = BaseLoginMethodSchema.extend({
  ...
  phoneNumber: z
    ...
    .optional()
    .describe(
      "Phone number used for passwordless login. Required if email is not provided"
    ),
  email: z
    .string()
    .optional()
    .describe(
      "Email used for passwordless login. Required if phoneNumber is not provided"
    ),
})
  .refine(
...
const LoginMethodSchema = z
  ...
  .describe(
    "Login method, one of Email/Password, Third Party, or Passwordless"
  );
...
export const CreateUserParametersSchema = z.object({
  ...
  userMetadata: z
    ...
    .optional()
    .describe(
      "Array of user metadata JSON objects. The array items valid JSON objects"
    ),
  userRoles: z.array(UserRoleSchema).optional().describe("Array of user roles"),
  totpDevices: z
    .array(TotpDeviceSchema)
    .optional()
    .describe("Array of TOTP devices"),
  loginMethods: z.array(LoginMethodSchema).describe("Array of login methods"),
  appId: z.string().optional().default("public").describe("App ID"),
});

type CreateUserParameters = z.infer<typeof CreateUserParametersSchema>;

async function createUserHandler(
  { appId, ...user }: CreateUserParameters,
  config: ToolContext
) {
  ...
  if (!response.ok) {
    throw new MCPServerError(`Failed to create user`, "MCP_TOOL_CALL_ERROR");
  }
  ...
  if (data.status !== "OK") {
    throw new MCPServerError(
      `Failed to create user - ${JSON.stringify(data)}`,
      "MCP_TOOL_CALL_ERROR"
    );
  }
...
export const CreateUserTool: Tool = {
  ...
  ## Input:
  - externalUserId (optional): ${CreateUserParametersSchema.shape.externalUserId.description}
  - userMetadata (optional): ${CreateUserParametersSchema.shape.userMetadata.description}
  - userRoles (optional):  ${CreateUserParametersSchema.shape.userRoles.description}  
    - role (required): ${UserRoleSchema.shape.role.description}
    - tenantIds (optional): ${UserRoleSchema.shape.tenantIds.description}
  - totpDevices (optional): ${CreateUserParametersSchema.shape.totpDevices.description}
  ...
  - loginMethods (required): ${CreateUserParametersSchema.shape.loginMethods.description}
  ...
  `,
  input: CreateUserParametersSchema,
