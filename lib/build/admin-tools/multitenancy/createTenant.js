...
const CreateTenantParametersSchema = zod_1.z.object({
    ...
    firstFactors: zod_1.z
        ...
        .optional()
        .describe("Optional list of first factors to enable for this tenant. Items can be one of: emailpassword, thirdparty, passwordless, otp-email, link-phone, link-email"),
    requiredSecondaryFactors: zod_1.z
        .array(schemas_1.AuthenticationFactorSchema)
        .optional()
        .describe("Optional list of required secondary factors for this tenant. Items can be one of: emailpassword, thirdparty, passwordless, otp-email, link-phone, link-email"),
    coreConfig: schemas_1.CoreConfigSchema.optional(),
});
async function createTenantHandler({ tenantId, firstFactors, requiredSecondaryFactors, coreConfig, }) {
    ...
    if (result.status !== "OK") {
        throw new error_1.MCPServerError(`Failed to create tenant - ${JSON.stringify(result)}`, "MCP_TOOL_CALL_ERROR");
    }
