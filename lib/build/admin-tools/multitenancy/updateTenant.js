...
async function updateTenantHandler({ tenantId, firstFactors, requiredSecondaryFactors, coreConfig, }) {
    ...
    if (result.status !== "OK") {
        throw new error_1.MCPServerError(`Failed to update tenant - ${JSON.stringify(result)}`, "MCP_TOOL_CALL_ERROR");
    }
