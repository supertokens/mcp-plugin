...
const RemoveUserFromTenantParametersSchema = zod_1.z.object({
    ...
    loginMethods: zod_1.z
        ...
        .describe("Which authentication methods should be dissasociated from the tenant. If not specified all of the users login methods will get included. Provide an array with some of the following values: emailpassword, thirdparty, passwordless, webauthn."),
});
async function removeUserFromTenantHandler({ userId, tenantId, loginMethods, }) {
    ...
    for (const recipe of loginMethods) {
        ...
        const result = await multitenancy_1.default.disassociateUserFromTenant(tenantId, recipeUserId);
        if (result.status !== "OK") {
