...
const AddUserToTenantParametersSchema = zod_1.z.object({
    userId: zod_1.z.string().uuid().describe("The ID of the user to add to the tenant"),
    ...
    loginMethods: zod_1.z
        ...
        .describe("Which authentication methods should be associated with the tenant. If not specified all of the users login methods will get included. Provide an array with some of the following values: emailpassword, thirdparty, passwordless, webauthn."),
});
async function addUserToTenantHandler({ userId, tenantId, loginMethods, }) {
    ...
    for (const recipe of loginMethods) {
        const recipeUserId = user.loginMethods.find((loginMethod) => loginMethod.recipeId === recipe)?.recipeUserId;
        if (!recipeUserId) {
            throw new error_1.MCPServerError(`User does not have ${recipe} as an authentication method`, "MCP_TOOL_CALL_ERROR");
        }
        const result = await multitenancy_1.default.associateUserToTenant(tenantId, recipeUserId);
        if (result.status !== "OK") {
            throw new error_1.MCPServerError(`Failed to add user to tenant. Error: ${JSON.stringify(result)}`, "MCP_TOOL_CALL_ERROR");
        }
    }
