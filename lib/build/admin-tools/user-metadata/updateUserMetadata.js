...
const UpdateUserMetadataParametersSchema = zod_1.z.object({
    ...
    metadata: zod_1.z
        ...
        .describe("The metadata object that will be associated with the user. It needs to be a JSON object with basic values."),
});
async function updateUserHandler({ userId, metadata, }) {
    ...
    const result = await usermetadata_1.default.updateUserMetadata(userId, metadata);
    if (result.status !== "OK") {
        throw new error_1.MCPServerError("Failed to update user metadata", "MCP_TOOL_CALL_ERROR");
    }
