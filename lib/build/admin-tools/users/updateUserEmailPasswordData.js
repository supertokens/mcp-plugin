...
const UpdateUserEmailPasswordDataParametersSchema = zod_1.z
    ...
    password: zod_1.z
        .string()
        .optional()
        .describe("The password used to identify the user. Omit the argument if you have specified the email."),
})
    .refine((data) => {
...
async function updateUserHandler({ userId, email, password, }) {
    ...
    const emailPasswordRecipeUserId = user?.loginMethods?.find((loginMethod) => loginMethod.recipeId === "emailpassword")?.recipeUserId;
    if (!emailPasswordRecipeUserId) {
        throw new error_1.MCPServerError("The user does not have emailpassword as an authentication method", "MCP_TOOL_CALL_ERROR");
    }
    ...
    if (result.status !== "OK") {
        throw new error_1.MCPServerError(`Failed to update email/password. Error: ${JSON.stringify(result)}`, "MCP_TOOL_CALL_ERROR");
    }
