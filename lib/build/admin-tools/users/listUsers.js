...
const ListUsersParametersSchema = zod_1.z.object({
    ...
    sortingOrder: zod_1.z
        .enum(["ASC", "DESC"])
        .default("ASC")
        .describe("The order in which the users are sorted. ASC - oldest users first, DESC - newest users first"),
    limit: zod_1.z
        .number()
        .default(100)
...
async function listUsersHandler({ sortingOrder, limit, paginationToken, tenantId, }) {
    let result;
    if (sortingOrder === "ASC") {
