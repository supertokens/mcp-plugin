...
const GetUserParametersSchema = zod_1.z
    ...
    email: zod_1.z
        ...
        .email()
        .optional()
        .describe("The email used to identify the user. Omit the argument if you have specified the userId or the phoneNumber"),
    phoneNumber: zod_1.z
        .string()
        .optional()
        .describe("The phone number used to identify the user. Omit the argument if you have specified the userId or the email"),
    userId: zod_1.z
        .string()
        .optional()
        .describe("The user ID used to identify the user. Omit the argument if you have specified the email or the phoneNumber"),
...
async function getUserHandler({ email, phoneNumber, userId, tenantId, }) {
    ...
    if (userId) {
        ...
        const users = await supertokens_node_1.default.listUsersByAccountInfo(tenantId, {
            email,
            phoneNumber,
        });
        user = users[0];
        ...
        throw new error_1.MCPServerError("Please provide either userId, email, or phoneNumber", "MCP_TOOL_CALL_ERROR");
    }
    if (!user) {
