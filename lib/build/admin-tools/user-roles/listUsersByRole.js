...
const ListUsersByRoleParametersSchema = zod_1.z.object({
    role: zod_1.z.string().describe("The role that will be used to filter the users"),
...
async function listUsersByRoleHandler({ role, tenantId, }) {
    ...
    if (result.status !== "OK") {
        throw new error_1.MCPServerError(`Failed to list roles - ${JSON.stringify(result)}`, "MCP_TOOL_CALL_ERROR");
    }
    return result;
}
