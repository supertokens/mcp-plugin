...
async function removeUserRolePermissionHandler({ role, permissions, }) {
    const result = await userroles_1.default.removePermissionsFromRole(role, permissions);
    if (result.status !== "OK") {
        throw new error_1.MCPServerError(`Failed to remove permissions from role - ${JSON.stringify(result)}`, "MCP_TOOL_CALL_ERROR");
    }
