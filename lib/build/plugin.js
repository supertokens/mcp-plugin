...
function default_1(pluginConfig) {
    ...
    return {
                ...
                functions: (oI) => {
                    return {
                        ...
                        buildAccessTokenPayload: async (input) => {
                            ...
                            const session = await session_1.default.getSessionInformation(input.sessionHandle, input.userContext);
                            ...
                            for (const claim of claims) {
                                const claimValue = await claim.fetchValue(userId, recipeUserId, tenantId, payload, input.userContext);
                                payload = claim.addToPayload_internal(payload, claimValue, input.userContext);
                            }
                            return payload;
                        },
                    };
        ...
        init: (config) => {
            pluginInterface = (0, pluginInterfaceImpl_1.default)(config.appInfo, pluginConfig);
        ...
        routeHandlers: () => {
            ...
            routeHandlers.push({
                ...
                handler: async (_req, res, _session, userContext) => {
                    let resp = await pluginInterface.wellKnownOAuthAuthorizationServer(userContext);
            ...
            routeHandlers.push({
                ...
                handler: async (_req, res, _session, userContext) => {
                    let resp = await pluginInterface.wellKnownOAuthProtectedResource(userContext);
                    res.sendJSONResponse(resp);
                    return null;
                },
            });
            routeHandlers.push({
                path: pluginConfig?.oauth?.registrationEndpointPath ?? "/oauth/register",
                method: "post",
                verifySessionOptions: { sessionRequired: false },
                handler: async (req, res, _session, userContext) => {
                    const response = await pluginInterface.registerOAuthClient(await req.getBodyAsJSONOrFormData(), userContext);
            ...
            const validateRegistrationAccessToken = async (req, clientId, clientSecret, userContext) => {
                ...
                return await pluginInterface.validateRegistrationAccessToken(clientId, clientSecret, accessToken, userContext);
            };
            routeHandlers.push({
                ...
                handler: async (req, res, _session, userContext) => {
                    ...
                    const response = await pluginInterface.getOAuthClient(clientId, userContext);
                    if (response.status === "OK") {
                        if (!(await validateRegistrationAccessToken(req, clientId, response.client_secret, userContext))) {
            ...
            routeHandlers.push({
                ...
                handler: async (req, res, _session, userContext) => {
                    ...
                    const clientResponse = await pluginInterface.getOAuthClient(clientId, userContext);
                    if (clientResponse.status === "OK") {
                        if (!(await validateRegistrationAccessToken(req, clientId, clientResponse.client_secret, userContext))) {
                            res.setStatusCode(401);
                            res.sendJSONResponse({
                                status: "ERROR",
                                error: "unauthorized",
                    ...
                    const response = await pluginInterface.updateOAuthClient(clientId, await req.getBodyAsJSONOrFormData(), userContext);
                    if (response.status === "OK") {
                        res.setStatusCode(200);
                        res.sendJSONResponse(response);
                    }
            ...
            routeHandlers.push({
                ...
                handler: async (req, res, _session, userContext) => {
                    ...
                    const response = await pluginInterface.deleteOAuth2Client(clientId, userContext);
                    if (response.status === "OK") {
                        res.setStatusCode(200);
                        res.sendJSONResponse(response);
                    }
