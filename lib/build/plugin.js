...
const verifySessionForMCP = (server, next) => {
  return async (req, res, session, userContext) => {
    ...
    let payload = {};
    try {
      payload = (await oauth2provider_1.default.validateOAuth2AccessToken(jwt))
        .payload;
    } catch (err) {
      ...
      for (const validator of server.claimValidators) {
        if ("claim" in validator) {
          ...
          const claimValue = await claim.fetchValue(
            payload.sub,
            new supertokens_node_1.RecipeUserId(payload.rsub),
            payload.tId,
            payload,
            userContext
          );
