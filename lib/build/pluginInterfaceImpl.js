...
function default_1(pluginConfig) {
    return {
        getAuthBaseUrl: function (appInfo) {
            return (new normalisedURLDomain_1.default(appInfo.apiDomain).getAsStringDangerous() +
                (appInfo.apiBasePath !== undefined
                    ? new normalisedURLPath_1.default(appInfo.apiBasePath).getAsStringDangerous()
                    : ""));
        },
        ...
        registerOAuthClient: async function (_appInfo, supertokensInfo, client) {
            ...
            const response = await fetch(`${supertokensInfo.connectionURI}/recipe/oauth/clients`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: supertokensInfo.apiKey !== undefined
