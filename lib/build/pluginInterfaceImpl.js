...
const normalisedURLDomain_1 = __importDefault(require("supertokens-node/lib/build/normalisedURLDomain"));
...
function default_1(appInfo, pluginConfig) {
    const apiDomain = new normalisedURLDomain_1.default(appInfo.apiDomain).getAsStringDangerous();
    const registrationPath = new normalisedURLPath_1.default(pluginConfig?.oauth?.registrationEndpointPath ?? "/oauth/register").getAsStringDangerous();
    const registrationEndpoint = `${apiDomain}${registrationPath}`;
    const clientsPath = new normalisedURLPath_1.default(pluginConfig?.oauth?.clientsEndpointPath ?? "/oauth/clients").getAsStringDangerous();
    const clientsEndpoint = `${apiDomain}${clientsPath}`;
    return {
        getRegistrationAccessTokenForClient: async function (clientId, clientSecret, _userContext) {
            const secret = pluginConfig?.oauth?.registrationAccessTokenSecret ?? "default-secret";
        ...
        validateRegistrationAccessToken: async function (clientId, clientSecret, accessToken, _userContext) {
            const secret = pluginConfig?.oauth?.registrationAccessTokenSecret ?? "default-secret";
            const hash = crypto_1.default
                .createHash("sha256")
                .update(`${clientId}:${clientSecret}:${secret}`)
        ...
        wellKnownOAuthProtectedResource: async function (_userContext) {
            ...
            return {
                ...
                authorization_servers: [
                    `${apiDomain}${new normalisedURLPath_1.default(appInfo.apiBasePath ?? "/auth").getAsStringDangerous()}`,
                ],
        ...
        registerOAuthClient: async function (client, userContext) {
            ...
            let response = await oauth2provider_1.default.createOAuth2Client(client, userContext);
            if (response.status === "OK") {
                const registrationAccessToken = await this.getRegistrationAccessTokenForClient(response.client.clientId, response.client.clientSecret, userContext);
        ...
        getOAuthClient: async function (clientId, userContext) {
            const response = await oauth2provider_1.default.getOAuth2Client(clientId, userContext);
            if (response.status === "OK") {
                return {
                    status: "OK",
                    client_id: response.client.clientId,
        ...
        updateOAuthClient: async function (clientId, clientUpdate, userContext) {
            ...
            const response = await oauth2provider_1.default.updateOAuth2Client({ clientId, ...clientUpdate }, userContext);
            if (response.status === "OK") {
        ...
        deleteOAuth2Client: async function (clientId, userContext) {
            return await oauth2provider_1.default.deleteOAuth2Client({ clientId }, userContext);
        },
    };
}
